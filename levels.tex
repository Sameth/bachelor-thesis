...
hrane vieme vypočítať iba z označení jej koncových vrcholov.
    ...
    $2^0:$  Položíme $s' := a_1 a_2 \cdots a_{n-1}$, čiže $s$ skrátený o posledný znak. 
            ...
            \begin{itemize}
                ...
                \item $V_0 := (a_{n-k+2} a_{n-k+3} \cdots a_n) \in V(G)$ a $sled_{k,S}(s')$ je neprázdny. Položíme \\
                      ...
                      čiže začiatok poslednej $k-1$-tice zodpovedajúcej nejakému vrcholu v $G$ a $s'' := a_1 a_2 \cdots a_{j+k-2}$,
                      čiže $s'$ skrátené po koniec poslednej $k-1$-tice zodpovedajúcej nejakému vrcholu v $G$. Slová $s'$ a $s''$ majú
                      ...
                      $slovo(sled_{k,S}(s)) = slovo(V_1) slovo(e_1) slovo(e_2) \cdots slovo(e_p) = slovo(sled_{k,S}(s'')) \cdot slovo(e_p)$.
                      ...
                      čo by bolo ale v spore s tým, že prekryv $V_0$ s $V_q$ je definovaný ako najdlhší možný a teda dokončenie $V_0$ vo $V_q$
                      je najkratšie možné. Aj v tomto prípade teda platí indukčný krok.
            \end{itemize}
