...
Prvým a najmenej zložitým problémom je hľadanie najkratšieho $k$-nadslova množiny $S$. Na pripomenuie,
ide o nadslovo, ktoré obsahuje všetky podslová dĺžky $k$ zo slov v $S$.
...
ľubovoľnému $k$-nadslovu a dokážeme jej vlastnosti.

\begin{defn}
    Nech $s = x_1 x_2 \cdots x_n$ je $k$-nadslovom množiny $S$, nech $G$ je $k$-grafom množiny $S$, nech
    $Z = \{ i \in \{1 \ldots n - k + 2 \} | (x_i x_{i+1} \cdots x_{i+k-2}) \in V(G) \}$
    a nech $v_i$ je $i$-ty najmenší prvok množiny $Z$. Potom \textit{sled nadslova $s$} určíme ako
    $$sled_S(s) = V_1 e_1 V_2 e_2 \cdots e_{p-1} V_p,$$
    kde $V_i = (x_{v_i} x_{v_i + 1} \cdots x_{v_i + k - 2})$ a $e_i$ je hrana medzi $V_i$ a $V_{i+1}$. 
\end{defn}

\begin{veta}
    Nech $s = x_1 x_2 \cdots x_n$ je $k$-nadslovom množiny $S$. Potom aj $slovo(sled_S(s))$ je
...
\begin{proof}
    ...
    $$\exists i : a_1 = x_i, a_2 = x_{i+1}, \ldots a_k = x_{i+k-1}$$
    ...
    $x_{i+1}$, čiže $sled(s)$ obsahuje podsled $U_1 e U_2$ a teda $slovo (sled(s))$ obsahuje $q$.
\end{proof}

\begin{veta}
    Nech $s = a_1 a_2 \cdots a_k$ je $k$-nadslovom množiny $S$. Potom 
    $$|slovo(sled_S(s))| \leq |s|.$$
\end{veta}
...
\begin{veta}
    ...
    $slovo(W) = s$, tak existuje slovo $s'$ také, že $s'$ je $k$-nadslovom množiny $S$ a zároveň $|s'| < |s|$.
\end{veta}

\begin{proof}
    V prvom rade si popíšeme, ako budeme konštruovať sled v našom $k$-grafe k zadanému $k$-nadslovu 
    $$s = x_1 x_2 \cdots x_n.$$
    Prvým vrcholom bude 
\end{proof}
