...
\begin{veta}
    Nech $S$ je množina slov nad abecedou $\Sigma$, $n$ je prirodzené a $k$ je kladné celé číslo, nech $s = a_1 a_2 \cdots a_n$ je
    nejakým slovom nad $S$. Potom $|slovo(sled_{k,S}(s))| \leq |s|.$
\end{veta}

\begin{proof}
    Nech $G$ je $k$-graf množiny $S$. Dokážeme matematickou indukciou na $n$, dĺžku slova $s$.

    $1^0:$  $n = 0$. Keďže $k > 0 = n$, množina $Z$ z konštrukcie $k$-sledu bude prázdna, teda aj $sled_{k,S}(s)$ bude
            prázdny, čiže $slovo (sled_{k,S}(s)) = \varepsilon, |\varepsilon| \leq 0$.
