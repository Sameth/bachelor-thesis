...
graf s vopred určeným počiatočným aj koncovým vrcholom, v ktorom každý vrchol $V$
okrem koncového spĺňa $\delta_O(V) > 1$. Pre daný orientovaný graf $G$ vieme
skonštruovať nový graf napríklad rozdelením niektorého vrchola na počiatočný vrchol $s$ a
koncový vrchol $t$ tak, že počiatočný si zachová všetky vychádzajúce a koncový
všetky vchádzajúce hrany. Pre dodržanie veľkosti stupňa pridáme ešte tri vrcholy
$t', a, b$, pridáme hrany z každého vrchola do $t'$ a navyše hrany $(t, a), (t, b),
(a, b), (b, a), (a, t')$. Tento nový graf obsahuje hamiltonovskú cestu práve vtedy,
...
hamiltonovskej cesty s nanajvýš $n$ vrcholmi.

Dôležitou vlastnosťou konštrukcie
inštancie rozhodovacieho problému nadslova z inštancie problému orientovanej
hamiltonovskej cesty je, že stačí, aby každý reťazec mal dĺžku $3$. Ďalšou dobrou
vlastnosťou je, že konštrukcia používa $(2n + 2)$-znakovú abecedu.
...
Pri uvedenom kódovaní má každý kód pôvodného znaku dĺžku presne $\frac{k}{3}$ a každé
slovo pri konštrukcii inštancie problému $k$-nadslova pri 
